...
5. The fifth block is the **`class stock`** which will be used to create an object for each ticker
...
6. The last block is the `create_current_summary_table function` which is used to build up the table on the basis of scrapped info.
```{python}
def create_current_summary_table():
   """
   data = read_scraped_data
   ranked_Data = aggregate_info(data)
   to_sql(data)
   """
   pass
```

The overall flow is like below:
...
DiagrammeR::grViz(
  ...
  digraph nicegraph{
    {main} -> update
    update -> run_by_historical_multiprocesses
    run_by_historical_multiprocesses -> run_update_historical_data
    run_update_historical_data -> Stock_run
    Stock_run -> create_current_summary_table
    create_current_summary_table -> main
  }
  ", height = 600
)
```
