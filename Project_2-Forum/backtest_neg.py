    ...
    def preprocess(self):
        try:
            if self.sentiment is None:
                self.stocks_post_matrix = pd.read_csv(f'data/interim/weekend_stocks_post_matrix_after_'
                                                      f'masking_{self.signal_period}.csv',
                                                      index_col=0, parse_dates=True)
            elif self.sentiment == 'positive':
                self.stocks_post_matrix = pd.read_csv(f'data/interim/weekend_stocks_post_matrix_after_'
                                                      f'masking_positive_{self.signal_period}.csv',
                                                      index_col=0, parse_dates=True)
            else:
                self.stocks_post_matrix = pd.read_csv(f'data/interim/weekend_stocks_post_matrix_after_'
                                                      f'masking_negative_{self.signal_period}.csv',
                                                      index_col=0, parse_dates=True)
            ...
            with Bar('CrossSignal Preprocessing', max=len(files)) as bar:
                for ticker, file in zip(tickers, files):
                    ...
                    curr_stock_posts_vec_half = df.groupby(pd.Grouper(freq='30T', base=30)).count()['Title']
                    curr_stock_posts_vec.name = ticker
                    new_curr_stock_posts_vec = curr_stock_posts_vec.groupby(pd.Grouper(freq='24H', base=14)).sum()

                    # This is to make the Hour from 15 to 0AM
                    new_curr_stock_posts_vec = new_curr_stock_posts_vec.resample('D').sum()

                    # Critical step is to add one day to the vector since this will be used to
                    # predict the return of next day
                    new_curr_stock_posts_vec.index = pd.DatetimeIndex(new_curr_stock_posts_vec.index) + pd.DateOffset(1)
                    new_curr_stock_posts_vec = new_curr_stock_posts_vec.rolling(window=self.signal_period,
                                                                                min_periods=1).sum()
                    for date in new_curr_stock_posts_vec.index[self.signal_period:]:
                        new_curr_stock_posts_vec.loc[date] = new_curr_stock_posts_vec.loc[date] - \
                                                             curr_stock_posts_vec_half[date + dt.timedelta(hours=14)
                                                                                  + dt.timedelta(minutes=30) -
                                                                                  dt.timedelta(days=self.signal_period)]

                    # This is to fix the date range
                    new_curr_stock_posts_vec = new_curr_stock_posts_vec[
                        (new_curr_stock_posts_vec.index >= self._start) & (new_curr_stock_posts_vec.index <= self._end)]
                    self.stocks_post_matrix = pd.concat([self.stocks_post_matrix, new_curr_stock_posts_vec], axis=1)

                    # self.stocks_post_matrix = self.stocks_post_matrix.fillna(0)
                # self.create_csi300_mask_matrix()
                if self.sentiment is None:
                    self.stocks_post_matrix.to_csv(f'data/interim/stocks_post_matrix_{self.signal_period}.csv')

                    """Handle the weekend posts"""
                    self.add_weekend_posts()
                    self.delete_zeros_weekend()
                    self.stocks_post_matrix.to_csv(f'data/interim/weekend_stocks_post_matrix_{self.signal_period}.csv')
                    self.create_csi300_mask_matrix()
                    self.stocks_post_matrix.to_csv(f'data/interim/weekend_stocks_post_matrix_'
                                                   f'after_masking_{self.signal_period}.csv')
                else:
                    self.stocks_post_matrix.to_csv(f'data/interim/stocks_post_matrix'
                                                   f'_{self.sentiment.lower()}_{self.signal_period}.csv')

                    """Handle the weekend posts"""
                    self.add_weekend_posts()
                    self.delete_zeros_weekend()
                    self.stocks_post_matrix.to_csv(f'data/interim/weekend_stocks_post_matrix'
                                                   f'_{self.sentiment.lower()}_{self.signal_period}.csv')
                    self.create_csi300_mask_matrix()
                    self.stocks_post_matrix.to_csv(f'data/interim/weekend_stocks_post_matrix_'
                                                   f'after_masking_{self.sentiment.lower()}_{self.signal_period}.csv')
